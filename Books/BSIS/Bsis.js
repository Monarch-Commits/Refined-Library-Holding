...
const bsisRef = ref(db, 'Books/BSIS');
...
function renderPagination() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    btn.className = `pageBtn  h-8 w-8 items-center hidden sm:flex justify-center rounded text-xs font-bold transition-all ${
      i === currentPage
...
window.deleteBook = function (year, bookId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      const bookRef = ref(db, `Books/BSIS/${year}/${bookId}`);
