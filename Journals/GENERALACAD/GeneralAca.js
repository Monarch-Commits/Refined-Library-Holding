...
const bsisRef = ref(db, 'Journals/GENERAL ACADEMIC');
      ...
      tableBody.innerHTML = `
        <tr>
          <td colspan="11" class="px-6 py-6 text-center text-slate-400">
            No GENERAL ACADEMIC records found
          </td>
        </tr>`;
      books = [];
      filteredBooks = [];
...
window.deleteBook = function (year, bookId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      const bookRef = ref(db, `Journals/GENERAL ACADEMIC/${year}/${bookId}`);
